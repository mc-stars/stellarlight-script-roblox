...
btn.Size=UDim2(0,50,0,22)
btn.Position=UDim2(1,-60,0.5,-11)
...
if enabled then notify(text.." 已开启") coro=task.spawn(func,true)
else notify(text.." 已关闭") if coro then task.cancel(coro) end func(false) end
end)
btn.Parent=frame
frame.Parent=parent
...
BtnText.Size=UDim2.new(0.7,0,0.7,0)
...
BtnText.Rotation=-30
BtnText.Parent=FloatButton

local BtnClick=Instance.new("TextButton")
BtnClick.BackgroundTransparency=1
BtnClick.Size=UDim2.new(1,0,1,0)
BtnClick.Parent=FloatButton

local dragFloat=false
BtnClick.MouseButton1Down:Connect(function()dragFloat=true end)
UserInputService.InputChanged:Connect(function(input)
if dragFloat and input.UserInputType==Enum.UserInputType.MouseMovement then
FloatButton.Position=UDim2.new(0,input.Position.X-35,0,input.Position.Y-35)
end
end)
UserInputService.InputEnded:Connect(function()dragFloat=false end)
...
MainWindow:TweenSize(UDim2.new(0,0,0,0),Enum.EasingDirection.In,Enum.EasingStyle.Quad,0.3,true)
task.wait(0.3)
MainWindow.Visible=false
end
end)
